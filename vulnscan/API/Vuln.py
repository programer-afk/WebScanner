...
import requests
from Web_Scanner.settings import API_TIMEOUT
...
class Vuln(Base):
    ...
    def get_all(self, status):
        ...
        try:
            response = requests.get(vuln_get_all_api, headers=self.auth_headers, verify=False, timeout=API_TIMEOUT)
    ...
    def get(self, vuln_id):
        ...
        try:
            response = requests.get(vuln_get_api, headers=self.auth_headers, verify=False, timeout=API_TIMEOUT)
            return response.json()
        except Exception:
    ...
    def search(self, severity, criticality, status, cvss_score=0.0, target_id=None, group_id=None):
        ...
        try:
            response = requests.get(vuln_search_api, headers=self.auth_headers, verify=False, timeout=API_TIMEOUT)
            # print(response.text)
