...
from Web_Scanner.settings import API_KEY, API_URL
import time
...
def vuln_result(request, target_id):
    try:
        ...
        # 添加调试信息
        print(f"正在查询目标ID: {target_id} 的漏洞")
        ...
        while retry_count < max_retries:
            try:
                ...
                retry_count += 1
                print(f"查询尝试 {retry_count}/{max_retries} 失败: {str(retry_error)}")
                if retry_count == max_retries:
                    raise Exception(f"多次尝试查询失败: {str(retry_error)}")
                time.sleep(2)  # 等待2秒后重试
        
        print(f"API返回数据: {vuln_details}")
        
        if not vuln_details or 'vulnerabilities' not in vuln_details or not vuln_details['vulnerabilities']:
            print("未找到漏洞数据")
            # 返回空数据列表和提示信息
            return render(request, 'vuln-reslut.html', {'data': data, 'info_message': '未找到漏洞数据，扫描可能仍在进行中或尚未发现漏洞'})
        ...
        print(f"漏洞结果查询出错: {str(e)}")
        error_msg = f"查询出错: {str(e)}"
        if "timed out" in str(e).lower():
            error_msg = "连接AWVS服务器超时，请确认服务器状态并重试"
        elif "connectionerror" in str(e).lower():
            error_msg = "无法连接到AWVS服务器，请确认服务器地址和API Key设置是否正确"
        # 返回空数据列表和详细错误消息
        return render(request, 'vuln-reslut.html', {'data': [], 'error_message': error_msg})
